...
export function useAgentAudioVisualizerWave({
  ...
  // FIX: Swapped out standard useState for your custom animated value hook
  ...
  useEffect(() => {
    switch (state) {
      ...
      case "connecting":
      case "initializing":
        animateSpeed(DEFAULT_SPEED * 4, DEFAULT_TRANSITION);
      ...
      case "speaking":
      default:
        animateSpeed(DEFAULT_SPEED * 2, DEFAULT_TRANSITION);
        animateAmplitude(DEFAULT_AMPLITUDE, DEFAULT_TRANSITION);
        animateFrequency(DEFAULT_FREQUENCY, DEFAULT_TRANSITION);
        animateOpacity(1.0, DEFAULT_TRANSITION);
        return;
    }
  }, [state, animateSpeed, animateAmplitude, animateFrequency, animateOpacity]);
