...
const Dashnavlist = ({ mobile, onLinkClick }: DashnavProps) => {
  ...
  return (
    <section className={`flex ${mobile ? "flex-col gap-1" : "flex-row justify-around"} w-full`}>
      {dashnav.map((nav) => (
        <div
          className={`${pathname === nav.link ? (mobile ? "bg-gray-100 font-medium text-[#0F172A]" : "bg-surface text-[#0F172A] font-medium") : (mobile ? "text-gray-600 hover:bg-gray-50" : "text-gray-
